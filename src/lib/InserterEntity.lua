...
function InserterEntity.copy_filter(source,dest)
	...
	--[[TRACE]]trace("copy_filter", source.name, dest.name, serpent.line(filter))
end
...
function InserterEntity.connect_wires(entity1, entity2)
	if(not entity1 or not entity2) then return end

	-- Check if a green/red cable already exists
	local green_wire_exists = false
	local red_wire_exists = false
	local connections = entity1.circuit_connection_definitions
	for _, connection in ipairs(connections) do
		if connection.source_entity == entity1 and 	connection.target_entity == entity2 then
			if connection.wire == defines.wire_type.green then green_wire_exists = true end
			if connection.wire == defines.wire_type.red then red_wire_exists = true end
		end
	end

	-- If no green cable exists, then create one
	if not green_wire_exists then
		local connected = entity1.connect_neighbour{
			target_entity = entity2,
			wire = defines.wire_type.green
		}
		--[[TRACE]]trace("connect_wire", entity1.name, entity2.name, "connected: "..tostring(connected))
	end

	-- If no red cable exists, then create one
	if not red_wire_exists then
		local connected = entity1.connect_neighbour{
			target_entity = entity2,
...
--TODO: move to utils, ...see Trace.defines_name
...
function InserterEntity.copy_behavior(entity1,entity2)
	--[[TRACE]]trace("copy_connection_conditions", entity1.name, entity2.name)
...
function InserterEntity.copyToTable(entity, t)
	...
	t.filter_slot_count = entity.filter_slot_count
	for i = 1, entity.filter_slot_count, 1 do ---@cast i uint
		if(i==1) then t.filters = {} end
		t.filters[i] = entity.get_filter(i)
	end	
	t.filter_mode = entity.inserter_filter_mode
	t.inserter_stack_size_override = entity.inserter_stack_size_override
